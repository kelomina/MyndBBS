...
| 阶段 | 内容 | 状态 |
|---|---|---|
| **Phase 1** | G1 用户举报系统（详细设计见 §3）+ 徽章联动 | ✅ 本轮实施 |
| **Phase 2** | G2 IP 封禁管理 + G3 新用户防灌水规则 | 规划中 |
| **Phase 3** | G4 Tag 标签 → G5 @提及 → G8 邮件通知 | 规划中 |
| **Phase 4** | G10 图片上传 → G11 站点设置 → G12 统计 → G13 SEO → G14 精华 | 清单 |
