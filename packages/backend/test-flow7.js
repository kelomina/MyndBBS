...
(async () => {
  try {
    ...
    // Root elevates user
    await request(`/api/admin/users/${newUser.id}/role`, 'PATCH', { role: 'ADMIN' }, rootCookie);

    // SSR Profile fetch (ignores set-cookie)
    let profileRes = await request('/api/v1/user/profile', 'GET', null, userCookie);
    console.log('SSR Profile:', profileRes.status);

    // Wait 61 seconds? No, user said they clicked it. So it's within 60 seconds.
    // CSR Admin users fetch with OLD cookie!
