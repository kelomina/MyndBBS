...
interface VerifiedAccessSession {
  userId: string;
  role: string;
  sessionId: string;
}

function getBearerOrCookieToken(req: AuthRequest): string | undefined {
...
function assertAccessTokenPayload(decoded: AccessTokenPayload): {
  userId: string;
  role: string;
  sessionId: string;
} {
  if (decoded.type !== 'access') {
    throw new Error('ERR_INVALID_ACCESS_TOKEN_TYPE');
  }
  ...
  if (
    ...
    throw new Error('ERR_INVALID_TOKEN_PAYLOAD');
  }

  return {
    userId: decoded.userId,
    role: typeof decoded.role === 'string' && decoded.role ? decoded.role : 'USER',
...
function isNonRefreshableAuthError(error: unknown): boolean {
  const errorCode = getAuthErrorCode(error);
  return errorCode === 'ERR_SESSION_REVOKED_OR_INVALID' ||
    errorCode === 'ERR_USER_BANNED' ||
    errorCode === 'ERR_ACCOUNT_NOT_ACTIVE';
}

export const verifyAccessTokenSession = async (token: string): Promise<VerifiedAccessSession> => {
  const decoded = jwt.verify(token, process.env.JWT_SECRET as string, { algorithms: ['HS256'] }) as AccessTokenPayload;
  const payload = assertAccessTokenPayload(decoded);

  const cachedSession = await authCache.getSessionValidity(payload.sessionId);
  if (cachedSession === 'invalid') {
    throw new Error('ERR_SESSION_REVOKED_OR_INVALID');
  }

  // 这里每次都重新查会话和用户状态。缓存只用来快速拒绝已知无效会话，
  // 避免“用户已被封禁但旧 valid 缓存仍放行”的安全窗口。
  const { isValid, roleName, reason } = await authApplicationService.validateSession(
    payload.sessionId,
    payload.userId,
  );

  if (!isValid) {
    await authCache.setSessionValidity(payload.sessionId, 'invalid', 300);
    throw sessionErrorForReason(reason);
  }

  await authCache.setSessionValidity(payload.sessionId, 'valid', 3600);
  return {
...
async function attachAuthenticatedContext(req: AuthRequest, verified: VerifiedAccessSession): Promise<void> {
  req.user = verified;
...
export const requireAuth = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
  ...
  try {
    const verified = await verifyAccessTokenSession(token);

    const requiresRefresh = await authCache.checkRequiresRefresh(verified.sessionId);
    if (requiresRefresh) {
      const newAccessToken = authApplicationService.generateAccessToken(
        verified.userId,
        verified.role,
        verified.sessionId,
      );
      ...
      await authCache.extendRefreshGracePeriod(verified.sessionId, 60);
    }

    await attachAuthenticatedContext(req, verified);
    next();
  } catch (error) {
    if (isNonRefreshableAuthError(error)) {
      res.clearCookie('accessToken');
      res.clearCookie('refreshToken');
      res.status(401).json({ error: getAuthErrorCode(error) });
      return;
    }
    ...
    if (refreshTokenValue) {
      try {
        const { accessToken: newAccessToken } =
          await authApplicationService.refreshAccessToken(refreshTokenValue);
        ...
        const verified = assertAccessTokenPayload(decoded);
