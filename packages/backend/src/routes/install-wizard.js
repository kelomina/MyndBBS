  ...
  window.handleAdminSubmit = async function(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        var redirectUrl = (document.getElementById('frontendUrl').value || appConfig.frontendUrl).replace(/\/+$/, '') + '/admin-setup';
        setTimeout(function() {
          fetch('/install/api/restart', { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + installToken } }).catch(function() {});
        }, 2000);
        setTimeout(function() { window.location.href = redirectUrl; }, 8000);
        return;
      } else {
