...
export class PrivateMessage {
  ...
  public get expiresInMs(): number | null { return this.props.expiresInMs; }
  public get expiresStartedAt(): Date | null { return this.props.expiresStartedAt; }
  public get autoDeleteForSenderAfterRead(): boolean { return this.props.autoDeleteForSenderAfterRead; }
  ...
  public startReceiverExpirationOnRead(now: Date = new Date()): void {
    if (!this.props.expiresInMs || this.props.expiresInMs <= 0 || this.props.expiresStartedAt) {
      return;
    }
    this.props.expiresStartedAt = now;
    this.props.expiresAt = new Date(now.getTime() + this.props.expiresInMs);
  }

  public deleteForSenderAfterRead(): boolean {
    if (!this.props.autoDeleteForSenderAfterRead) {
      return false;
    }
    return this.deleteForUser(this.props.senderId, false);
  }
