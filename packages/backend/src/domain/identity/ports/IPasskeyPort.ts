...
export interface IPasskeyPort {
  ...
  verifyRegistrationResponse(response: any, expectedChallenge: string, expectedOrigin: string | string[], expectedRPID: string): Promise<any>;
  generateAuthenticationOptions(allowCredentials: any[]): Promise<any>;
  verifyAuthenticationResponse(response: any, expectedChallenge: string, expectedOrigin: string | string[], expectedRPID: string, credential: any): Promise<any>;
}
