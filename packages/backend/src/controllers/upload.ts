...
export const handleFileUpload = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    const ext = path.extname(req.file.originalname).slice(1) || req.file.mimetype.split('/')[1] || 'enc';
    const url = await systemApplicationService.uploadAttachment(req.file.buffer, ext);
    res.json({ url });
  } catch (error: any) {
