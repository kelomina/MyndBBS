...
export const verifyEmailRegistration = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    res.cookie('tempToken', tempToken, getAuthCookieOptions(60 * 60 * 1000, 'strict'));
  ...
  try {
    ...
    try {
      ...
      if (error.message.startsWith('ERR_')) {
        const statusCode = ['ERR_ACCOUNT_IS_BANNED', 'ERR_ADMIN_SECOND_FACTOR_REQUIRED'].includes(error.message) ? 403 : 401;
        res.status(statusCode).json({ error: error.message });
        return;
      }
      throw error;
    ...
    if (authResult.requires2FA && authResult.tempToken) {
      res.cookie('tempToken', authResult.tempToken, getAuthCookieOptions(60 * 60 * 1000));
