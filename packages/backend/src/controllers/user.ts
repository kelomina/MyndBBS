...
export const updateEmailNotifications = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await userApplicationService.updateEmailNotifications(userId, enabled);
...
export const getBookmarkedPosts = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const unifiedBookmarks = await identityQueryService.listBookmarks(userId, req.ability!);
...
export const generatePasskeyOptions = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    console.error('[generatePasskeyOptions] 500 error:', error.message || error);
    res.status(500).json({ error: 'ERR_INTERNAL_SERVER_ERROR' });
  }
};
  ...
  try {
    ...
    try {
      ...
      const errorCode = typeof error?.message === 'string' && error.message.startsWith('ERR_')
        ? error.message
        : 'ERR_PASSKEY_VERIFICATION_FAILED';
      res.status(400).json({ error: errorCode });
      return;
    }
  } catch (error) {
    console.error('[verifyPasskey] 500 error:', error instanceof Error ? error.message : error);
    res.status(500).json({ error: 'ERR_INTERNAL_SERVER_ERROR' });
  }
};
...
export const uploadAvatar = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (dotIndex === -1 || dotIndex === req.file.originalname.length - 1) {
      res.status(400).json({ error: 'ERR_INVALID_FILENAME' });
      return;
    }
    const ext = req.file.originalname.toLowerCase().slice(dotIndex + 1);
    const avatarUrl = await userApplicationService.updateAvatar(userId, req.file.buffer, ext);
    res.json({ avatarUrl });
  } catch (error: any) {
    if (error?.message?.startsWith('ERR_')) {
      res.status(400).json({ error: error.message });
...
export const deleteAvatar = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    await userApplicationService.removeAvatar(userId);
