...
export const updateUserRole = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (error.message.startsWith('ERR_')) {
      ...
      console.error('[updateUserRole] 500 error:', error.message || error);
...
export const deleteUser = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    const user = await adminUserManagementApplicationService.anonymizeUser(
      { userId: operatorId, role: (req.user?.role || 'USER') as any },
      id
    );
    ...
    if (error.message.startsWith('ERR_')) {
      ...
      console.error('[deleteUser] 500 error:', error.message || error);
      res.status(500).json({ error: 'ERR_INTERNAL_SERVER_ERROR' });
    }
  }
};

export const createTestAccount = async (req: AuthRequest, res: Response): Promise<void> => {
  const operatorId = req.user?.userId || 'unknown';

  try {
    const user = await adminUserManagementApplicationService.createTestAccount(
      { userId: operatorId, role: (req.user?.role || 'USER') as any },
      {
    ...
    res.status(201).json({ message: 'Test account created', user });
  } catch (error: any) {
    if (error.message.startsWith('ERR_')) {
      const statusCode = error.message.includes('FORBIDDEN') ? 403 : 400;
      res.status(statusCode).json({ error: error.message });
    } else {
      console.error('[createTestAccount] 500 error:', error.message || error);
      res.status(500).json({ error: 'ERR_INTERNAL_SERVER_ERROR' });
    }
  }
};
...
export const deleteCategory = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (error.message === 'ERR_CATEGORY_NOT_FOUND') {
      res.status(404).json({ error: 'ERR_CATEGORY_NOT_FOUND' });
    } else if (error.message === 'ERR_CATEGORY_NOT_EMPTY') {
      res.status(400).json({ error: 'ERR_CATEGORY_NOT_EMPTY' });
    } else {
      res.status(500).json({ error: 'ERR_SERVER_ERROR' });
    }
  }
...
export const getPublicRouteWhitelist = async (req: Request, res: Response) => {
  try {
    const routes = await systemQueryService.listPublicRouteWhitelist();
    res.json(routes);
  } catch (error) {
    res.status(500).json({ error: 'ERR_FAILED_TO_FETCH_ROUTE_WHITELIST' });
  }
