...
export const approvePendingPost = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  try {
    await moderationApplicationService.approvePost(id, operatorId, req.ability);
    res.json({ message: 'Post approved' });
  } catch (error: any) {
    if (error.message === 'ERR_FORBIDDEN_INSUFFICIENT_PERMISSIONS_TO_MANAGE_THIS_POST') {
      res.status(403).json({ error: error.message });
      return;
    }
...
export const rejectPendingPost = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const operatorId = req.user!.userId;
  try {
    await moderationApplicationService.rejectPost(id, reason, operatorId, req.ability);
...
export const approvePendingComment = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const operatorId = req.user!.userId;
  try {
    await moderationApplicationService.approveComment(id, operatorId, req.ability);
...
export const rejectPendingComment = async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const operatorId = req.user!.userId;
  try {
    await moderationApplicationService.rejectComment(id, operatorId, req.ability);
