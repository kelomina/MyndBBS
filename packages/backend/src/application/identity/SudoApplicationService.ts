 ...
 */
export interface SudoApplicationServiceOptions {
  userSecurityReadModel: IUserSecurityReadModel
  authApplicationService: AuthApplicationService
  sudoStore: ISudoStore
  getRpID: () => string
  getOrigin: () => string
  totpPort: ITotpPort
}
export class SudoApplicationService {
  constructor(private readonly opts: SudoApplicationServiceOptions) {}
  ...
  public async getPasskeyOptions(userId: string): Promise<any> {
    const userPasskeys = await this.opts.userSecurityReadModel.listUserPasskeyIds(userId);
    if (userPasskeys.length === 0) throw new Error('ERR_NO_PASSKEYS_REGISTERED');

    const options = await generateAuthenticationOptions({
      rpID: this.opts.getRpID(),
    ...
    const authChallenge = await this.opts.authApplicationService.generateAuthChallenge(options.challenge);
  ...
  public async verify(userId: string, sessionId: string, input: VerifyInput): Promise<void> {
    const user = await this.opts.userSecurityReadModel.getUserWithRoleById(userId);
    ...
    if (input.type === 'totp') {
      ...
      const isValidTotp = this.opts.totpPort.verify(user.totpSecret, input.totpCode);
      isValid = isValidTotp;
    }

    if (input.type === 'passkey') {
      const expectedChallenge = await this.opts.authApplicationService.consumeAuthChallenge(input.challengeId);
      const passkey = await this.opts.userSecurityReadModel.getPasskeyById(input.passkeyResponse.id);
      if (!passkey || passkey.userId !== userId) throw new Error('ERR_INVALID_PASSKEY');

      const verification = await verifyAuthenticationResponse({
        response: input.passkeyResponse,
        expectedChallenge: expectedChallenge.challenge,
        expectedOrigin: this.opts.getOrigin(),
        expectedRPID: this.opts.getRpID(),
      ...
      if (isValid && verification.authenticationInfo) {
        await this.opts.authApplicationService.updatePasskeyCounter(passkey.id, BigInt(verification.authenticationInfo.newCounter));
      }
    }

    if (!isValid) throw new Error('ERR_VERIFICATION_FAILED');
    await this.opts.sudoStore.grant(sessionId, 15 * 60);
  }
  ...
  public async check(sessionId: string): Promise<boolean> {
    return this.opts.sudoStore.check(sessionId);
  }
}
