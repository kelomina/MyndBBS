...
export interface AuthApplicationServiceOptions {
  ...
  authCache: ISessionCache
  totpPort: ITotpPort
  passkeyPort: IPasskeyPort
...
export class AuthApplicationService {
  ...
  public async registerUser(
    ...
    const hashedPassword = await this.opts.passwordHasher.hash(password)
    ...
    await this.opts.emailRegistrationTicketRepository.save(ticket)
  ...
  public async resendEmailRegistration(email: string): Promise<RegistrationRequestAcceptedResult> {
    const normalizedEmail = EmailAddress.create(email.trim().toLowerCase()).value
    const existingUser = await this.opts.userRepository.findByEmail(normalizedEmail)
    if (existingUser) {
    ...
    await this.opts.emailRegistrationTicketRepository.save(replacementTicket)

    try {
      await this.sendRegistrationVerificationEmail(replacementTicket)
    } catch (error) {
      await this.opts.emailRegistrationTicketRepository.delete(replacementTicket.id)
      throw error
    }
  ...
  public async verifyEmailRegistration(verificationToken: string): Promise<RegisteredUserResult> {
    const ticket = await this.opts.emailRegistrationTicketRepository.findByVerificationToken(
    ...
    const registeredUser = await this.opts.unitOfWork.execute(async () => {
      const existingEmailUser = await this.opts.userRepository.findByEmail(ticket.email)
      const existingUsernameUser = await this.opts.userRepository.findByUsername(ticket.username)
      ...
      if (existingEmailUser || existingUsernameUser) {
        if (
          ...
          const existingRole = existingEmailUser.roleId
            ? await this.opts.roleRepository.findById(existingEmailUser.roleId)
            : null
  ...
  public async requestPasswordReset(email: string): Promise<PasswordResetRequestAcceptedResult> {
    ...
    await this.opts.passwordResetTicketRepository.save(ticket)
  ...
  public async resetPasswordWithToken(resetToken: string, newPassword: string): Promise<void> {
    ...
    const ticket = await this.opts.passwordResetTicketRepository.findByResetToken(resetToken.trim())
    if (!ticket) {
    ...
    await this.opts.unitOfWork.execute(async () => {
      const user = await this.opts.userRepository.findById(ticket.userId)
      if (!user || user.email !== ticket.email) {
        throw new Error('ERR_USER_NOT_FOUND')
      }

      const hashedPassword = await this.opts.passwordHasher.hash(newPassword)
      user.updateProfile(undefined, undefined, hashedPassword)
      await this.opts.userRepository.save(user)
      await this.opts.sessionRepository.deleteManyByUserId(user.id)
    })
  ...
  public async changePasswordWithVerification(
    ...
    return this.opts.unitOfWork.execute(async () => {
      ...
      if (newEmail && newEmail !== user.email) {
        const existing = await this.opts.userRepository.findByEmail(newEmail)
        if (existing) throw new Error('ERR_EMAIL_ALREADY_IN_USE')
      }
      if (newUsername && newUsername !== user.username) {
        const existing = await this.opts.userRepository.findByUsername(newUsername)
        if (existing) throw new Error('ERR_USERNAME_ALREADY_IN_USE')
      }

      let hashedPassword
      if (newPassword) {
        hashedPassword = await this.opts.passwordHasher.hash(newPassword)
      }
  ...
  public async loginUser(
    ...
    let user = await this.opts.userRepository.findByEmail(emailOrUsername)
    if (!user) {
      user = await this.opts.userRepository.findByUsername(emailOrUsername)
    }
    ...
    const isValid = await this.opts.passwordHasher.verify(user.password, password)
    if (!isValid) {
      throw new Error('ERR_INVALID_CREDENTIALS')
    }
    ...
    const passkeys = await this.opts.passkeyRepository.findByUserId(user.id)
    ...
    if (roleName && BACKOFFICE_ROLE_NAMES.has(roleName) && methods.length === 0) {
      throw new Error('ERR_ADMIN_SECOND_FACTOR_REQUIRED')
    }
  ...
  public async generatePasskeyRegistrationOptions(userId: string): Promise<any> {
    ...
    const userPasskeys = await this.opts.passkeyRepository.findByUserId(userId)
  ...
  public async verifyPasskeyRegistration(
    ...
    return this.opts.unitOfWork.execute(async () => {
      ...
      const verification = await this.opts.passkeyPort.verifyRegistrationResponse(
        response,
  ...
  public async createSession(
    ...
    await this.opts.sessionRepository.save(session)
    return session
  }
  ...
  public async generateTotp(
    ...
    const secret = this.opts.totpPort.generateSecret()
    const otpauth = this.opts.totpPort.generateURI(APP_NAME, email, secret)
  ...
  public async storeTotpSecret(
    ...
    await this.opts.authCache.storeTotpSecret(userId, secret, ttlSeconds)
  }
  ...
  public async verifyTotpRegistration(userId: string, code: string): Promise<string> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      const isValid = this.opts.totpPort.verify(pendingSecret, code)
      if (!isValid) {
  ...
  public async getTotpSecret(userId: string): Promise<string | null> {
    return await this.opts.authCache.getTotpSecret(userId)
  }
  ...
  public async removeTotpSecret(userId: string): Promise<void> {
    await this.opts.authCache.removeTotpSecret(userId)
  }
  ...
  public async revokeAllUserSessions(userId: string): Promise<void> {
    await this.opts.sessionRepository.deleteManyByUserId(userId)
  }
  ...
  public async validateSession(
    ...
    if (user.status !== UserStatus.ACTIVE) {
      return { isValid: false, reason: 'USER_NOT_ACTIVE' }
    }
  ...
  public async generateAuthChallenge(
    ...
    await this.opts.authChallengeRepository.save(challenge)
    return challenge
  }
  ...
  public async consumeAuthChallenge(challengeId: string): Promise<AuthChallenge> {
    const challenge = await this.opts.authChallengeRepository.findById(challengeId)
    if (!challenge) {
      throw new Error('ERR_CHALLENGE_NOT_FOUND')
    }

    challenge.validateForConsumption()
    await this.opts.authChallengeRepository.delete(challengeId)
  ...
  public async generatePasskeyAuthenticationOptions(userId?: string): Promise<any> {
    ...
    const options = await this.opts.passkeyPort.generateAuthenticationOptions(allowCredentials)
    const authChallenge = await this.generateAuthChallenge(options.challenge)
  ...
  public async verifyPasskeyAuthenticationResponse(
    ...
    return this.opts.unitOfWork.execute(async () => {
      ...
      const passkey = await this.opts.passkeyRepository.findById(response.id)
      if (!passkey) {
      ...
      const verification = await this.opts.passkeyPort.verifyAuthenticationResponse(
        response,
        expectedChallenge.challenge,
        getOrigin(),
        getRpID(),
      ...
      if (verification.verified && verification.authenticationInfo) {
        ...
        const user = await this.opts.userRepository.findById(authenticatedUserId)
        if (!user) throw new Error('ERR_USER_NOT_FOUND')
        if (user.status === UserStatus.BANNED) {
          throw new Error('ERR_ACCOUNT_IS_BANNED')
        }
  ...
  public async verifyTotpLogin(userId: string, code: string): Promise<any> {
    ...
    const isValid = this.opts.totpPort.verify(user.totpSecret, code)
    if (!isValid) {
      throw new Error('ERR_INVALID_TOTP_CODE')
    }
  ...
  public async getUserFromTempToken(
    ...
    try {
      ...
      if (!user) return null
      if (expectedType === 'login' && user.status !== UserStatus.ACTIVE) return null
  ...
  public verifyTempToken(token: string): any {
    return this.opts.tokenPort.verify(token, getTempTokenSecret())
  }
  ...
  public generateTempToken(userId: string, type: 'registration' | 'login'): string {
    return this.opts.tokenPort.sign({ userId, type }, getTempTokenSecret(), '1h')
  }
  ...
  public async addPasskey(
    ...
    await this.opts.passkeyRepository.save(passkey)
  }
  ...
  public async deletePasskey(id: string, requesterUserId: string): Promise<string> {
    const passkey = await this.opts.passkeyRepository.findById(id)
    if (!passkey) {
      throw new Error('ERR_PASSKEY_NOT_FOUND')
    }
    ...
    await this.opts.passkeyRepository.delete(id)

    const remainingPasskeys = await this.opts.passkeyRepository.findByUserId(requesterUserId)
    if (remainingPasskeys.length === 0) {
      const user = await this.opts.userRepository.findById(requesterUserId)
      if (user) {
  ...
  public async updatePasskeyCounter(credentialId: string, newCounter: bigint): Promise<string> {
    const passkey = await this.opts.passkeyRepository.findById(credentialId)
    if (!passkey) {
      throw new Error('ERR_PASSKEY_NOT_FOUND')
    }
  ...
  private async deleteReusablePendingRegistration(email: string, username: string): Promise<void> {
    ...
    if (
      ...
      await this.opts.emailRegistrationTicketRepository.delete(pendingByUsername.id)
    }
  }
  ...
  private async deleteReusablePasswordResetTicket(userId: string, email: string): Promise<void> {
    const pendingByUserId = await this.opts.passwordResetTicketRepository.findByUserId(userId)
    if (pendingByUserId) {
      await this.opts.passwordResetTicketRepository.delete(pendingByUserId.id)
    }

    const pendingByEmail = await this.opts.passwordResetTicketRepository.findByEmail(email)
    if (pendingByEmail && pendingByEmail.id !== pendingByUserId?.id) {
      await this.opts.passwordResetTicketRepository.delete(pendingByEmail.id)
    }
  }
  ...
  private async sendRegistrationVerificationEmail(ticket: EmailRegistrationTicket): Promise<void> {
    ...
    await this.opts.emailSender.sendEmail(command)
  }
  ...
  private async renderTemplateOrDefault(
    ...
    if (this.opts.emailTemplateRepository) {
      try {
        const template = await this.opts.emailTemplateRepository.findByType(type)
        if (template) {
  ...
  private buildEmailRegistrationVerificationLink(verificationToken: string, email: string): string {
    const frontendUrl = (process.env.FRONTEND_URL || getFirstOrigin()).replace(/\/+$/, '')
