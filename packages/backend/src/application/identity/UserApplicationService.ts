 ...
 */
export interface UserApplicationServiceOptions {
...
export class UserApplicationService {
  constructor(private readonly opts: UserApplicationServiceOptions) {}
  ...
  public async updateProfile(userId: string, email?: string, username?: string, hashedPassword?: string): Promise<any> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      if (email && email !== user.email) {
        const existing = await this.opts.userRepository.findByEmail(email);
        if (existing) throw new Error('ERR_EMAIL_ALREADY_IN_USE');
      }
      if (username && username !== user.username) {
        const existing = await this.opts.userRepository.findByUsername(username);
  ...
  public async changeRole(userId: string, roleId: string | null): Promise<void> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      await this.opts.userRepository.save(user);
      await this.opts.abilityCache.invalidateUserRules(userId);
    });
  }
  ...
  public async changeLevel(userId: string, level: number): Promise<void> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      const passkeys = await this.opts.passkeyRepository.findByUserId(userId);
  ...
  public async updateAvatar(userId: string, content: Buffer, ext: string): Promise<string> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      if (user.avatarUrl) {
        await this.opts.storagePort.deleteAvatar(user.avatarUrl);
      }

      const avatarUrl = await this.opts.storagePort.saveAvatar(userId, content, ext);
