 ...
 */
export interface RoleApplicationServiceOptions {
...
export class RoleApplicationService {
  constructor(private readonly opts: RoleApplicationServiceOptions) {}
  ...
  public async updateRole(roleId: string, name: string, description: string | null): Promise<Role> {
    return this.opts.unitOfWork.execute(async () => {
      const role = await this.opts.roleRepository.findById(roleId);
  ...
  public async createPermission(action: string, subject: string, conditions: string | null): Promise<Permission> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      await this.opts.permissionRepository.save(permission);
      return permission;
    });
  }

  private async invalidateCacheForRoleUsers(roleId: string): Promise<void> {
    const users = await this.opts.userRepository.findByRoleId(roleId);
    await this.opts.abilityCache.invalidateUsersRules(users.map(u => u.id));
  }
  ...
  public async assignPermissionToRole(roleId: string, permissionId: string): Promise<Role> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      const permission = await this.opts.permissionRepository.findById(permissionId);
