 ...
 */
export interface CommunityApplicationServiceOptions {
...
export class CommunityApplicationService {
   ...
   */
  constructor(private readonly opts: CommunityApplicationServiceOptions) {}
  ...
  private async getPostSubject(post: Post) {
    const category = await this.opts.categoryRepository.findById(post.categoryId)
    return subject('Post', {
      id: post.id,
      title: post.title,
  ...
  private async getCommentSubject(comment: Comment, postObj?: Post) {
    const post = postObj || (await this.opts.postRepository.findById(comment.postId))
    const category = post ? await this.opts.categoryRepository.findById(post.categoryId) : null
  ...
  public async createCategory(
    ...
    await this.opts.categoryRepository.save(category)
    await this.opts.eventBus.publish(new CategoryCreatedEvent(category.id, operatorId))
    return category
  }
  ...
  public async updateCategory(
    ...
    const category = await this.opts.categoryRepository.findById(id)
    ...
    await this.opts.categoryRepository.save(category)
    await this.opts.eventBus.publish(new CategoryUpdatedEvent(id, operatorId))
  }
  ...
  public async assignCategoryModerator(
    ...
    const isModerator = await this.opts.identityIntegrationPort.isModerator(userId)
    if (!isModerator) {
    ...
    await this.opts.categoryRepository.save(category)
    await this.opts.eventBus.publish(new CategoryModeratorAssignedEvent(categoryId, userId, operatorId))
  ...
  public async removeCategoryModerator(
    ...
    await this.opts.categoryRepository.save(category)
    await this.opts.eventBus.publish(new CategoryModeratorRemovedEvent(categoryId, userId, operatorId))
  }

  /**
   * Function: deleteCategory
   * -------------------------
   * 删除分类。为了避免破坏帖子、评论和审计记录的引用完整性，分类下仍有帖子时拒绝删除。
   *
   * Deletes a category. To preserve referential integrity for posts, comments, and audit records,
   * deletion is rejected while the category still contains posts.
   *
   * Callers: [AdminController.deleteCategory]
   * Called by: [AdminController.deleteCategory]
   *
   * Callees: [ICategoryRepository.findById, IPostRepository.countByCategoryId,
   *           ICategoryRepository.delete, IEventBus.publish]
   * Calls: [ICategoryRepository.findById, IPostRepository.countByCategoryId,
   *         ICategoryRepository.delete, IEventBus.publish]
   *
   * Parameters:
   * - id: string, 分类 ID / category ID
   ...
   * Error Handling / 错误处理:
   * - ERR_CATEGORY_NOT_FOUND: 分类不存在 / category not found
   * - ERR_CATEGORY_NOT_EMPTY: 分类下仍有帖子 / category still contains posts
   *
   * Side Effects / 副作用:
   * - 写入数据库（删除空分类）/ writes to database (deletes an empty category)
   * - 发布领域事件 / publishes a domain event
   *
   * Transaction / 事务:
   * - 单次删除；不存在级联硬删除 / single delete; no hard-delete cascade
   *
   * 中文关键词: 分类删除, 引用完整性, 非空拒绝, 领域事件, 审计
   * English keywords: category delete, referential integrity, non-empty reject, domain event, audit
   */
  public async deleteCategory(id: string, operatorId: string): Promise<void> {
    const category = await this.opts.categoryRepository.findById(id)
    if (!category) throw new Error('ERR_CATEGORY_NOT_FOUND')

    const postCount = await this.opts.postRepository.countByCategoryId(id)
    if (postCount > 0) {
      throw new Error('ERR_CATEGORY_NOT_EMPTY')
    }

    await this.opts.categoryRepository.delete(id)

    await this.opts.eventBus.publish(new CategoryDeletedEvent(id, operatorId))
  }
  ...
  public async createPost(
    ...
    const isModerated = await this.opts.moderationPolicy.containsModeratedWord(
      title + ' ' + content,
      categoryId,
    )
  ...
  public async deletePost(ability: AnyAbility, postId: string): Promise<void> {
    ...
    await this.opts.unitOfWork.execute(async () => {
      await this.opts.postRepository.save(post)
      await this.opts.commentRepository.softDeleteManyByPostId(postId)
    })
  }
  ...
  public async createComment(
    ...
    const postSubject = await this.getPostSubject(post)
    if (!ability.can('read', postSubject)) {
      throw new Error('ERR_FORBIDDEN_INSUFFICIENT_PERMISSIONS')
    }
    ...
    if (!ability.can('create', commentSubject)) {
      throw new Error('ERR_FORBIDDEN_INSUFFICIENT_PERMISSIONS')
    }

    let parentComment
    if (parentId) {
      parentComment = await this.opts.commentRepository.findById(parentId)
      if (!parentComment || parentComment.postId !== postId) {
        throw new Error('ERR_INVALID_PARENT_COMMENT')
      }
    }

    const isModerated = await this.opts.moderationPolicy.containsModeratedWord(content, post.categoryId)
  ...
  public async togglePostUpvote(
    ...
    const existing = await this.opts.engagementRepository.findPostUpvote(postId, userId)
    if (existing) {
      await this.opts.engagementRepository.deletePostUpvote(postId, userId)
      return false // Toggled off
    } else {
      const upvote = PostUpvote.create({ userId, postId, createdAt: new Date() })
      await this.opts.engagementRepository.savePostUpvote(upvote)
  ...
  public async togglePostBookmark(
    ...
    const existing = await this.opts.engagementRepository.findPostBookmark(postId, userId)
    if (existing) {
      await this.opts.engagementRepository.deletePostBookmark(postId, userId)
      return false
    } else {
      const bookmark = PostBookmark.create({ userId, postId, createdAt: new Date() })
      await this.opts.engagementRepository.savePostBookmark(bookmark)
      return true
    }
  }
  ...
  public async toggleCommentUpvote(
    ...
    const existing = await this.opts.engagementRepository.findCommentUpvote(commentId, userId)
    if (existing) {
      await this.opts.engagementRepository.deleteCommentUpvote(commentId, userId)
      return false
    } else {
      const upvote = CommentUpvote.create({ userId, commentId, createdAt: new Date() })
      await this.opts.engagementRepository.saveCommentUpvote(upvote)
      return true
    }
  }
  ...
  public async toggleCommentBookmark(
    ...
    const existing = await this.opts.engagementRepository.findCommentBookmark(commentId, userId)
    if (existing) {
      await this.opts.engagementRepository.deleteCommentBookmark(commentId, userId)
      return false
    } else {
      const bookmark = CommentBookmark.create({ userId, commentId, createdAt: new Date() })
      await this.opts.engagementRepository.saveCommentBookmark(bookmark)
      return true
    }
  }
}
