 ...
 */
export interface SystemApplicationServiceOptions {
  routeWhitelistRepository: IRouteWhitelistRepository
  storagePort: IStoragePort
  unitOfWork: IUnitOfWork
}
export class SystemApplicationService {
  constructor(private readonly opts: SystemApplicationServiceOptions) {}
   ...
   */
  public async uploadAttachment(content: Buffer, ext: string = 'enc'): Promise<string> {
    const safeExt = ext.toLowerCase().replace(/[^a-z0-9]/g, '') || 'enc';
    const filename = `${uuidv4()}.${safeExt}`;
    return this.opts.storagePort.saveFile(filename, content);
  }
  ...
  public async addRouteWhitelist(path: string, isPrefix: boolean, minRole: string | null, description: string | null): Promise<RouteWhitelist> {
    return this.opts.unitOfWork.execute(async () => {
      ...
      await this.opts.routeWhitelistRepository.save(route);
      return route;
    });
  }
  ...
  public async updateRouteWhitelist(id: string, path: string, isPrefix: boolean, minRole: string | null, description: string | null): Promise<RouteWhitelist> {
    return this.opts.unitOfWork.execute(async () => {
      const route = await this.opts.routeWhitelistRepository.findById(id);
  ...
  public async deleteRouteWhitelist(id: string): Promise<void> {
    return this.opts.unitOfWork.execute(async () => {
      const route = await this.opts.routeWhitelistRepository.findById(id);
      if (!route) throw new Error('ERR_ROUTE_NOT_FOUND');
      await this.opts.routeWhitelistRepository.delete(id);
    });
  }
}
