 ...
 */
export interface AuditApplicationServiceOptions {
  auditLogRepository: IAuditLogRepository
  unitOfWork: IUnitOfWork
}
export class AuditApplicationService {
  constructor(private readonly opts: AuditApplicationServiceOptions) {}
  ...
  public async logAudit(
    ...
    try {
      await this.opts.unitOfWork.execute(async () => {
        ...
        await this.opts.auditLogRepository.save(auditLog);
      });
    } catch (error) {
  ...
  public async getPaginatedLogs(params: {
    ...
    return this.opts.auditLogRepository.findMany(params);
  }
  ...
  public async cleanupOldLogs(retentionDays: number = 90): Promise<number> {
    ...
    return this.opts.auditLogRepository.deleteOlderThan(cutoff);
  }
}
