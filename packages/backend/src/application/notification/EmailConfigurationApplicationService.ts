...
import { assertAllowedOutboundHost, assertAllowedResolvedHost } from '../../lib/networkSecurity'
...
export const validateSmtpHostForOutboundConnection = (host: string): void => {
  try {
    assertAllowedOutboundHost(host)
  } catch {
    throw new Error('ERR_SMTP_HOST_NOT_ALLOWED')
  }
}

export const validateResolvedSmtpHostForOutboundConnection = async (host: string): Promise<void> => {
  try {
    await assertAllowedResolvedHost(host)
  } catch {
    throw new Error('ERR_SMTP_HOST_NOT_ALLOWED')
  }
}
 ...
 */
export interface EmailConfigurationApplicationServiceOptions {
  envStore: IEnvStore
  emailTemplateRepository: IEmailTemplateRepository
}
export class EmailConfigurationApplicationService {
  constructor(private readonly opts: EmailConfigurationApplicationServiceOptions) {}
  ...
  public async updateSmtpConfig(config: SmtpConfigInput, operatorRole?: string): Promise<void> {
    ...
    await this.opts.envStore.updateSmtpConfig(config)
  }
  ...
  public async getEmailTemplates(operatorRole?: string): Promise<EmailTemplateView[]> {
    ...
    const templates = await this.opts.emailTemplateRepository.findAll()
  ...
  public async updateEmailTemplate(
    ...
    await this.opts.emailTemplateRepository.upsert(template)
  }
  ...
  public async sendTestEmail(
    ...
    if (smtpConfig) {
      await validateResolvedSmtpHostForOutboundConnection(smtpConfig.host)
      ...
      if (savedHost) {
        await validateResolvedSmtpHostForOutboundConnection(savedHost)
      }
