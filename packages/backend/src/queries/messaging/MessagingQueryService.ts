...
export class MessagingQueryService {
  ...
  public async getMessages(userId: string, limit: number, cursor?: string, withUserId?: string): Promise<MessageListDTO> {
    ...
    const messages = await prisma.privateMessage.findMany({
      ...
      include: { sender: { select: { username: true, avatarUrl: true } }, receiver: { select: { username: true, avatarUrl: true } } }
    });
    ...
    return {
      messages: messages.reverse().filter((m: any) => {
        if (m.expiresAt && new Date(m.expiresAt) <= new Date()) {
          if (m.receiverId === userId) return false;
        }
        return true;
      }).map((m: any) => ({
        id: m.id,
        ...
        sender: { username: m.sender.username, avatarUrl: m.sender.avatarUrl },
        receiver: { username: m.receiver.username, avatarUrl: m.receiver.avatarUrl },
      })),
      nextCursor,
