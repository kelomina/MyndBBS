...
export class IdentityQueryService {
   ...
   */
  public async listBookmarks(userId: string, ability: AppAbility): Promise<BookmarkDTO[]> {
    ...
    const commentBookmarks = await prisma.commentBookmark.findMany({
      where: {
        ...
        comment: {
          AND: [
            rulesToPrisma(ability, 'read', 'Comment'),
            {
              post: {
                AND: [
                  { status: { in: NORMAL_READABLE_POST_STATUSES } },
  ...
  public async listSessions(userId: string): Promise<SessionDTO[]> {
    const list = await prisma.session.findMany({ where: { userId, expiresAt: { gt: new Date() } }, orderBy: { createdAt: 'desc' } });
