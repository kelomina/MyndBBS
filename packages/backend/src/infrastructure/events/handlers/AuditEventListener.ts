...
export class AuditEventListener {
  ...
  private registerHandlers(): void {
    ...
    this.eventBus.subscribe<UserDeletedEvent>('UserDeletedEvent', async (event) => {
      await this.auditApplicationService.logAudit(event.operatorId, 'DELETE_USER', `User:${event.targetUserId}`);
    });

    this.eventBus.subscribe<TestAccountCreatedEvent>('TestAccountCreatedEvent', async (event) => {
      await this.auditApplicationService.logAudit(event.operatorId, 'CREATE_TEST_ACCOUNT', `User:${event.targetUserId}`);
    });
