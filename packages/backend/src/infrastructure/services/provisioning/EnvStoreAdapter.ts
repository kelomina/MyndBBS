...
/**
 * 函数名称：serializeEnvValue
 *
 * 函数作用：
 *   将用户输入序列化为 .env 单行双引号值，防止换行、引号、反斜杠污染后续变量。
 * Purpose:
 *   Serializes user input as a single-line quoted .env value, preventing newline,
 *   quote, and backslash injection into following variables.
 */
export function serializeEnvValue(value: string): string {
  return JSON.stringify(String(value));
}

export function upsertSerializedKey(envContent: string, key: string, value: string): string {
  return upsertKey(envContent, key, serializeEnvValue(value));
}
...
export function upsertFrontendUrlOrigin(envContent: string, origin: string): string {
  ...
  if (!match) {
    return upsertSerializedKey(envContent, key, origin);
  }
  ...
  const next = serializeEnvValue(items.join(','));
...
export function applyDomainConfigToEnv(envContent: string, input: DomainConfigInput): string {
  ...
  let next = envContent;
  next = upsertSerializedKey(next, 'ORIGIN', origin);
  next = upsertSerializedKey(next, 'RP_ID', input.rpId);
...
export class EnvStoreAdapter implements IEnvStore {
  ...
  async updateDatabaseUrl(newDbUrl: string): Promise<void> {
    ...
    if (content.includes('DATABASE_URL=')) {
      content = content.replace(/^DATABASE_URL=.*$/m, `DATABASE_URL=${serializeEnvValue(newDbUrl)}`);
    } else {
      const suffix = content.endsWith('\n') || content.length === 0 ? '' : '\n';
      content += `${suffix}DATABASE_URL=${serializeEnvValue(newDbUrl)}\n`;
    }
  ...
  async setupEnvironment(
    ...
    let envContent = await this.read();

    envContent = upsertKey(envContent, 'PORT', String(config.port || 3001));
    envContent = upsertSerializedKey(envContent, 'FRONTEND_URL', frontendUrlValue);
  ...
  async updateSmtpConfig(config: SmtpConfigInput): Promise<void> {
    let content = await this.read();
    content = upsertSerializedKey(content, 'SMTP_HOST', config.host);
    content = upsertKey(content, 'SMTP_PORT', String(config.port));
    content = upsertKey(content, 'SMTP_SECURE', config.secure ? 'true' : 'false');
    content = upsertSerializedKey(content, 'SMTP_USER', config.user);
    content = upsertSerializedKey(content, 'SMTP_PASS', config.pass);
    content = upsertSerializedKey(content, 'SMTP_FROM', config.from);
    await this.write(content);
