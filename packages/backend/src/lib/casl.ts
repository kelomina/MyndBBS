...
export function defineAbilityForContext(context?: AccessContextDTO, extraRules?: RuleDescriptorDTO[]) {
  ...
  if (!context) {
    ...
    // Guest can read public Wikis
    can('read', 'Wiki', { isPublic: true, status: 'ACTIVE', minReadLevel: { $lte: 0 } } as any);
    can('read', 'WikiPage', { 'wiki.isPublic': true, 'wiki.status': 'ACTIVE', 'wiki.minReadLevel': { $lte: 0 } } as any);
    return build();
  }
  ...
  // Wiki permissions
  can('read', 'Wiki', { isPublic: true, status: 'ACTIVE', minReadLevel: { $lte: userLevel } } as any);
  can('read', 'Wiki', { ownerId: context.userId } as any);
  can('create', 'Wiki', undefined, { field: 'level', test: () => userLevel >= 2 });
  can('manage', 'Wiki', { ownerId: context.userId } as any);
  can('read', 'WikiPage', { 'wiki.isPublic': true, 'wiki.status': 'ACTIVE', 'wiki.minReadLevel': { $lte: userLevel } } as any);
  can('read', 'WikiPage', { 'wiki.ownerId': context.userId } as any);
