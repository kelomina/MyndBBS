...
test('supervisor replaces a plugin only after the new host is ready', async () => {
  ...
  const manifest = path.join(root, 'manifest.json'); await writeFile(manifest, JSON.stringify({ id: 'demo', version: '1.0.0', entry: 'entry.cjs', sha256: createHash('sha256').update(code).digest('hex') }))
