...
describe('PrivateMessage Aggregate Root', () => {
  ...
  describe('read lifecycle helpers', () => {
    it('should start receiver expiration from read time', () => {
      ...
      message.startReceiverExpirationOnRead(readAt);

      expect(message.expiresStartedAt).toEqual(readAt);
      expect(message.expiresAt).toEqual(new Date('2026-06-01T00:01:00.000Z'));
    });

    it('should delete only the sender when auto-delete after receiver read is enabled', () => {
      const message = PrivateMessage.create({
        ...getValidProps(),
        autoDeleteForSenderAfterRead: true,
      }, 2, true, 0);

      const shouldHardDelete = message.deleteForSenderAfterRead();

      expect(shouldHardDelete).toBe(false);
      expect(message.deletedBy).toEqual(['user-1']);
    });
  });
