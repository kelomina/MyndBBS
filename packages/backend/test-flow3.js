...
(async () => {
  try {
    ...
    // 4. Root accesses an endpoint to simulate "Session revoked or invalid"
    let rootAfterElevate = await request('/api/v1/user/profile', 'GET', null, rootCookie);
    console.log('Root profile after elevate:', rootAfterElevate.status, rootAfterElevate.data);
    ...
    // 6. User accesses profile
    let userProfile = await request('/api/v1/user/profile', 'GET', null, userCookie);
    console.log('User profile:', userProfile.status, userProfile.data);
