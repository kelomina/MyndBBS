...
test('Legal Pages exist and have correct routing', async (t) => {
  ...
  const routeGuardPath = path.join(process.cwd(), 'src', 'middleware', 'authGuard.ts');
  ...
  await t.test('Frontend proxy keeps legal pages public', async () => {
    const routeGuardContent = await fs.readFile(routeGuardPath, 'utf-8');
    assert.equal(routeGuardContent.includes("'/terms'"), true, 'Missing /terms from essential public paths');
    assert.equal(routeGuardContent.includes("'/privacy'"), true, 'Missing /privacy from essential public paths');
    assert.equal(routeGuardContent.includes("'/forgot-password'"), true, 'Missing /forgot-password from essential public paths');
    assert.equal(routeGuardContent.includes("'/reset-password'"), true, 'Missing /reset-password from essential public paths');
  });
});
