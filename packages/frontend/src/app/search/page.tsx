...
import { SearchRateLimitIsland } from '../../components/SearchRateLimitIsland'
...
export default async function SearchPage({
  ...
  if (q) {
    try {
      const res = await serverFetch(`/api/search?q=${encodeURIComponent(q)}`)
      if (res.ok) {
    ...
    <main className="mx-auto flex max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="flex-1 py-6 md:pl-8">
        <div className="mx-auto max-w-3xl space-y-8">
          ...
          {q && rateLimited ? (
            <SearchRateLimitIsland q={q} initialRetryAfterSec={rateLimited.retryAfterSec} dict={dict} />
          ) : (
            <>
              {q && posts.length === 0 && users.length === 0 && (
                <div data-testid="empty-state" className="text-center text-muted py-10">{dict.search.noResults}</div>
              )}
