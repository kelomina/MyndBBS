...
export default function FriendsPage() {
  ...
  const { toast } = useToast();
    ...
    try {
      const res = await fetchWithAuth('/api/v1/friends');
      if (res.ok) {
  ...
  const handleRespond = async (id: string, accept: boolean) => {
    try {
      await fetchWithAuth('/api/v1/friends/respond', {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      window.dispatchEvent(new CustomEvent('friendship-changed', {
        detail: { friendshipId: id, status: accept ? 'ACCEPTED' : 'REJECTED' }
      }));
      loadFriends();
    } catch (e) {
      console.error(e);
  ...
  const handleRemoveFriend = async (targetUserId: string) => {
    ...
    try {
      await fetchWithAuth('/api/v1/friends/remove', {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleBlockUser = async (targetUserId: string) => {
    ...
    try {
      await fetchWithAuth('/api/v1/friends/block', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
