...
export default function MessagesPage() {
  ...
  const [passwordPrompt, setPasswordPrompt] = useState<{ isOpen: boolean; message: string; resolve: (value: string | null) => void } | null>(null);
    ...
    try {
      const res = await fetchWithAuth('/api/v1/messages/inbox');
      if (res.ok) {
        ...
        (data.messages as InboxMessage[]).forEach((msg) => {
          ...
          const otherAvatarUrl = msg.senderId === myId ? msg.receiver.avatarUrl : msg.sender.avatarUrl;
          console.log('[Inbox] thread with:', otherUsername, 'avatarUrl:', otherAvatarUrl, 'sender:', msg.sender, 'receiver:', msg.receiver);

          let previewText = dict.messages?.encryptedMessage || 'Encrypted message';
          if (msg.isSystem && msg.encryptedContent) {
            try {
              const parsed = JSON.parse(msg.encryptedContent);
              if (parsed.content === '{{username}} wants to be your friend.' && parsed.params?.username) {
                previewText = dict.messages?.friendRequestReceived?.replace('{username}', parsed.params.username) || `${parsed.params.username} wants to be your friend.`;
          ...
          if (!threadsMap.has(otherUserId)) {
            threadsMap.set(otherUserId, {
              id: otherUserId,
              withUser: { id: otherUserId, username: otherUsername, avatarUrl: otherAvatarUrl },
              lastMessage: {
  ...
  const { connected } = useWebSocket({
    ...
    try {
      const [profileRes, keyRes] = await Promise.all([
        ...
        fetchWithAuth('/api/v1/messages/keys/me')
      ]);
  ...
  const handleInitSecureMessaging = async () => {
    ...
    try {
      ...
      // 5. Upload
      const uploadRes = await fetchWithAuth('/api/v1/messages/keys', {
        method: 'POST',
        headers: {
  ...
  if (userLevel < 2) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-12 text-center">
        ...
        <h2 className="text-2xl font-bold mb-2">{dict.messages?.accessDenied || 'Access Denied'}</h2>
        <p className="text-muted">{dict.messages?.levelTooLowDesc || 'You must be at least Level 2 to use private messages.'}</p>
      </div>
    );
  }
          ...
          {threads.length === 0 ? (
            ...
            <div className="rounded-xl border border-border bg-card shadow-sm overflow-hidden flex flex-col">
              {threads.map((thread) => (
                <Link
                  key={thread.id}
                  href={`/messages/${encodeURIComponent(thread.withUser.username)}`}
                ...
                >
                  <div className="flex items-center gap-4">
                    <Avatar src={thread.withUser.avatarUrl} username={thread.withUser.username === 'system' ? (dict.messages?.systemMessages || 'System') : thread.withUser.username} size={40} />
                    <div>
                      <h3 className="font-semibold text-foreground">{thread.withUser.username === 'system' ? (dict.messages?.systemMessages || 'System Messages') : thread.withUser.username}</h3>
                      <p className="text-xs text-muted mt-0.5">
                        {thread.lastMessage.previewText || (dict.messages?.encryptedMessage || 'Encrypted message')} • {new Date(thread.lastMessage.createdAt).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
